...
def train_gpt_model(
    ...
    print(f"training on {device} with {dtype} data type")
    print(f"gradient accumulation: {gradient_accumulation_steps} steps")
    print(f"batch size: {batch_size}")
    print(f"block size: {block_size}")
    print(  f"total tokens per iter: {tokens_per_iter:,}")
    print(f"total iters: {max_iters}")
    print(f"total tokens: {tokens_per_iter * max_iters:,}")
    # division by zero error if input_data is empty
    print(f"total epochs: {max_iters * batch_size * gradient_accumulation_steps / len(input_data):.2f}")
    ...
    print(  f"always save checkpoint: {always_save_checkpoint}")
    print(f"eval only: {eval_only}")
    print(f"log interval: {log_interval}")
    # log input data
    print(f"input data has {len(input_data):,} tokens")
    print(f"input data has {len(input_data)//block_size:,} blocks") 
    print(f"input data has {len(input_data)//(block_size*batch_size):,} batches")
    print(f"input data has {len(input_data)//(block_size*batch_size*gradient_accumulation_steps):,} gradient accumulation steps")   
