...
from shared.data import system_message, assistant_message, user_message
from shared.discord import get_channel_by_id, get_channel_by_name, get_random_and_latest_messages, send_message, split_markdown, get_latest_channel_docs
from shared.mongodb import discord_message_collection, timmy_answer_cache_collection
...
async def handle_chunk(chunk,stream_id,finished=False):
    ...
    if finished or state.get("new_message"):
        ...
        update_state({ "discussion":[*state['discussion'],message_text], "new_message":"" })
        await send_message({ 
                "content":message_text, 
    ...
    if len(split)>1:
        for message_text in split[:-1]:
            update_state({ "discussion":[*state['discussion'],message_text]})
            await send_message({ "content":message_text, "channel":state['channel_id'] },sent_messages,client,mark_sent=False)
...
async def main():
    ...
    update_state({ "channel_id":channel_choice.id })
    topics=await ask_docs(
        f"List all topics that were discussed in {channel_choice.name}.",
        state=update_state({
            "channel_name":channel_choice.name,
